jQuery(document).ready(function($){
	//Drag resizing
	if($('#black_piano-logo-adjuster').length > 0){
		var resizeRatio = $('#black_piano-logo-adjuster').attr('class').split('-');
		resizeRatio = resizeRatio[2] / resizeRatio[1];
		$('#black_piano-logo-adjuster img').draggable({
			containment: 'parent',
			drag: function(){
		...
		$('#dp-adjust-realvalue').click(function(e){
			...
			$('#black_piano-logo-adjuster img').css({
				top: top + 'px',
				left: left + 'px'
			});
		});
