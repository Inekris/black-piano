...
$option = get_black_piano_option(); //オプションの値を取得
if($logo){
...
function dp_logo_img_tag($resized = false, $alt = '', $aditional_class = '', $max_width = 0){
	$class = 'black_piano-logo ';
	$class .= $resized ? 'black_piano-logo-resized' : 'black_piano-logo-original';
...
function dp_logo_basedir(){
	$dir = wp_upload_dir();
	return $dir['basedir'].DIRECTORY_SEPARATOR.'black-piano';
}
...
function dp_logo_baseurl(){
	$dir = wp_upload_dir();
	return $dir['baseurl'].'/black-piano';
}
...
function _dp_upload_logo(){
	...
	if(!file_exists($dir) || !is_dir($dir)){
		...
		if(!@mkdir($dir)){
			$dp_upload_error = array(
				'error' => true,
				'message' => sprintf(
					__('Cannot create upload directory. Please make sure <code>%s</code> is writable.', 'black-piano'),
					dirname($dir)
				)
			);
			return $dp_upload_error;
	...
	if(!is_writable($dir)){
		$dp_upload_error = array(
			'error' => true,
			'message' => sprintf(
				__('Cannot save uploaded file. Please make sure <code>%s</code> is writable.', 'black-piano'),
				$dir
			)
		);
		return $dp_upload_error;
	...
	if(!preg_match("/(png|gif|jpe?g)$/i", $_FILES['dp_image']['name'], $ext)){
		$dp_upload_error = array(
			'error' => true,
			'message' => __('Uploaded file type is not allowed. Allowed file types are PNG, JPG and GIF.', 'black-piano')
		);
		return $dp_upload_error;
	}
	//既存のファイルを削除する
	...
	foreach($existing_files as $file){
		if(preg_match("/logo(-resized)?\.(png|gif|jpe?g)$/i", $file)){
			...
			if(!@unlink($dir.DIRECTORY_SEPARATOR.$file)){
				$dp_upload_error = array(
					'error' => true,
					'message' => sprintf(__('Cannot delete existing file <code>%s</code>', 'black-piano'), $file)
				);
				return $dp_upload_error;
			}
		}
	...
	if(!@move_uploaded_file($_FILES['dp_image']['tmp_name'], $dir.DIRECTORY_SEPARATOR.'logo.'.$ext[1])){
		$dp_upload_error = array(
			'error' => true,
			'message' => __('Sorry, but cannot save uploaded file.', 'black-piano')
		);
		return $dp_upload_error;
	}
	//ここまで来たということは保存に成功しているので、
	//メッセージを更新する
	$dp_upload_error['message'] = __('Logo file was successfully uploaded.', 'black-piano');
	return  $dp_upload_error;
}
...
function _dp_delete_message_error(){
	echo '<div id="message" class="error"><p>'.sprintf(__('Failed to delete image. Please check permisson of %s. All files must be writable.', 'black-piano'), dp_logo_basedir()).'</p></div>';
}
...
function _dp_delete_message_sucess(){
	echo '<div id="message" class="updated fade"><p>'.__('Images are successfully deleted.', 'black-piano').'</p></div>';
}
...
function _dp_resize_logo(){
	...
	$info = dp_logo_info();
	if(!$info){
		$dp_resize_message['message'] = __('Logo file not exists.', 'black-piano');
	...
	if($dest_path && !@unlink($dest_path)){
		$dp_resize_message['message'] = __('Cannot delete existing resized logo.', 'black-piano');
		return $dp_resize_message;
	}
	//名前を決めて保存
	...
	if($result){
		...
		$dp_resize_message['message'] = __('Logo image is successfully resized.', 'black-piano');
	}else{
		$dp_resize_message['message'] = sprintf(__('Failed to save resized logo. Please check permission of <code>%s</code>', 'black-piano'), dp_logo_basedir());
	}
	return $dp_resize_message;
}
...
function _dp_get_upload_err_msg($error_code){
	...
	case UPLOAD_ERR_INI_SIZE:
		return __('Uploaded file size is larger than limit set in php.ini.', 'black-piano');
		break;
	case UPLOAD_ERR_FORM_SIZE:
		return __('Uploaded file size is larger than post file size.', 'black-piano');
		break;
	case UPLOAD_ERR_PARTIAL:
		return  __('The file has been uploaded only partially. Connection error may have occured', 'black-piano');
		break;
	case UPLOAD_ERR_NO_FILE:
		return  __('Uploaded file size is too large.', 'black-piano');
		break;
	case UPLOAD_ERR_NO_TMP_DIR:
		return  __('No temporary directory exists. Please check PHP Setting is valid.', 'black-piano');
		break;
	case UPLOAD_ERR_CANT_WRITE:
		return  __('Failed to write to disk. OS file setting or something is wrong.', 'black-piano');
		break;
	case UPLOAD_ERR_EXTENSION:
		return  __('PHP extension module stops uploading. Check PHP setting.', 'black-piano');
		break;
	default:
		return  __('Upload failed. Sorry, but reasons cannot be detected.', 'black-piano');
		break;
	}
}